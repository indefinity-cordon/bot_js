...
async function backupTable(db, table_name) {
	...
	await globalThis.mysqlRequest(db, `CREATE TABLE ${backup_table_name} LIKE ${table_name}`);
	await globalThis.mysqlRequest(db, `INSERT INTO ${backup_table_name} SELECT * FROM ${table_name}`);
...
async function checkAndCreateTable(db, table_name, expected_structure) {
  const rows = await globalThis.mysqlRequest(
    db,
    `SHOW TABLES LIKE '${table_name}'`,
  );
  if (rows.length === 0) {
    console.log(
      `Database >> MySQL (ENTITY) >> [ERROR] >> Table ${table_name} does not exist. Creating...`,
    );
    //await createTable(db, table_name, expected_structure); NO REAL ACTIONS, JUST WARNING
  } else {
    const table_structure = await globalThis.mysqlRequest(
      db,
      `DESCRIBE ${table_name}`,
    );
    if (!compareTableStructure(table_structure, expected_structure)) {
      console.log(
        `Database >> MySQL (ENTITY) >> [ERROR] >> Table ${table_name} structure mismatch. Backing up and updating...`,
      );
...
async function createTable(db, table_name, structure) {
	...
	await globalThis.mysqlRequest(db, `CREATE TABLE \`${table_name}\` (${columns})`);
...
async function updateTable(db, table_name, expected_structure) {
	await globalThis.mysqlRequest(db, `DROP TABLE ${table_name}`);
...
function compareTableStructure(actual_structure, expected_structure) {
  ...
  return (
    JSON.stringify(actual_columns.sort()) ===
    JSON.stringify(expected_columns.sort())
  );
}

async function tables_tester(db) {
  for (const [, /*key*/ meta] of Object.entries(globalThis.entity_meta)) {
    await checkAndCreateTable(db, meta.table, meta);
  }
}

export default tables_tester;
