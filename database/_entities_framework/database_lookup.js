...
async function backupTable(db, table_name) {
	...
	await global.mysqlRequest(db, `CREATE TABLE ${backup_table_name} LIKE ${table_name}`);
	await global.mysqlRequest(db, `INSERT INTO ${backup_table_name} SELECT * FROM ${table_name}`);
...
async function checkAndCreateTable(db, table_name, expected_structure) {
	const rows = await global.mysqlRequest(db, `SHOW TABLES LIKE '${table_name}'`);
	if (rows.length === 0) {
...
//        await createTable(db, table_name, expected_structure); NO REAL ACTIONS, JUST WARNING
		...
		const table_structure = await global.mysqlRequest(db, `DESCRIBE ${table_name}`);
...
async function createTable(db, table_name, structure) {
	...
	await global.mysqlRequest(db, `CREATE TABLE \`${table_name}\` (${columns})`);
...
async function updateTable(db, table_name, expected_structure) {
	await global.mysqlRequest(db, `DROP TABLE ${table_name}`);
