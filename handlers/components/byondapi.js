			...
			return new Promise((resolve, reject) => {
				...
				client_api.on('data', (data) => {
					if (data.slice(0, 2).equals(Buffer.from([0x00, 0x83]))) {
						...
						if (packetType === 0x2a) {
							...
							console.log(`ByondAPI >> [ERROR] >> Unknown BYOND data code: 0x${packetType.toString(16)}`);
							reject();
						}
					} else {
						client.connections_in_proggress[`${port}:${address}`] = null;
						client_api.end();
						console.log('ByondAPI >> [ERROR] >> BYOND server returned invalid data.');
						reject();
					}
				});
				client_api.on('error', async (err) => {
					...
					if (!client.notified[`${port}:${address}`]) {
						...
						console.log(`ByondAPI >> [WARNING] >> Can't connect to ${address}:${port}: ${err.message}`);
					}
					resolve();
				});
			});
		} else {
			console.log(`ByondAPI >> [WARNING] >> Malformed ByondAPI request, with request: ${request}, target: ${address}:${port}.`);
		}
	}
}
