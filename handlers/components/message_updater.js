...
async function updateUpdatersMessages(client, game_server) {
    const db_request = await global.mysqlRequest(global.database, "SELECT type, channel_id, message_id FROM server_channels WHERE server = ? AND message_id NOT LIKE '-%'", [game_server.id]);
    ...
    for (const updater of db_request) {
        ...
        if (found_message === null) {
            await client.embed({
                ...
                global.mysqlRequest(global.database, "UPDATE server_channels SET message_id = ? WHERE server = ? AND type = ? AND channel_id = ?", [message.id, game_server.id, updater.type, updater.channel_id]);
            });
        }
