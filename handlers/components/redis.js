...
module.exports = (client) => {
    client.redisCallback = async function (data) {
        ...
        if (!responded_instance) {
            console.log(chalk.blue(chalk.bold(`Socket`)), chalk.white(`>>`), chalk.red(`[ERROR]`), chalk.white(`>>`), chalk.red(`Redis`), chalk.red(`Failed to find server instance. Aborting. data: ${data.source}`));
            return;
        }
        ...
        if (!responded_game_server) {
            console.log(chalk.blue(chalk.bold(`Socket`)), chalk.white(`>>`), chalk.red(`[ERROR]`), chalk.white(`>>`), chalk.red(`Redis`), chalk.red(`Failed to find server object. Aborting. data: ${JSON.parse(data)}, found objects: ${responded_instance.id}`));
            return;
        }
